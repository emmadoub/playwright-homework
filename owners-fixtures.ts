...
export type OwnersFixtures = {
    ownerSetUp: ownerInfo
    ownersPage: ownerInfo
}

export const test = base.extend<OwnersFixtures>({

 
    ownerSetUp: async ({ request }, use) => {
        ...
        const petBirthDate = faker.date.past().toISOString().slice(0, 10)
        const newPetResponse = await request.post(`https://petclinic-api.bondaracademy.com/petclinic/api/owners/${newOwnerID}/pets`, {
            data: {
                "id": null, "owner": newOwnerBody, "name": petName, "birthDate": petBirthDate, "pettype": "lizard", "type": { "name": "lizard", "id": 3075 }
            }
        })
       ...
       ownersPage: async ({ page, ownerSetUp }, use) => {

        await page.goto("/")
        await page.getByRole("button", { name: "Owners" }).click();
        await page.getByRole("link", { name: "Search" }).click();
        await use(ownerSetUp)
    }
